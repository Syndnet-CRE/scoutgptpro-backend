...
async function main() {
  ...
  console.log(`Found ${needEnrich.length.toLocaleString()} properties needing enrichment\n`);
  
  const ids = needEnrich.map(p => p.parcelId);
  const batches = Math.ceil(ids.length / BATCH);
  
  let totalUpdated = 0, totalNotInApi = 0;
  ...
  for (let i = 0; i < batches; i++) {
    ...
    try {
      ...
      console.log(`Got ${features.length}, Updated ${updated}, NotInAPI ${batchIds.length - features.length}`);
    ...
    if ((i+1) % 50 === 0) {
      ...
      console.log(`  >> Total: ${totalUpdated.toLocaleString()} updated, ${min.toFixed(1)}m elapsed`);
    }
  }
  ...
  console.log(`Properties with siteAddress: ${final.toLocaleString()} (${Math.round(final/total*100)}%)`);
